...
def plot_price_adjustment_simulation(result, df, grading_col, sim_params, formula_type='cosine'):
    ...
    one of several formula types.

    Args:
        result: A fitted statsmodels model with a continuous price interaction.
        df (pd.DataFrame): The original dataframe.
        grading_col (str): The grading column to simulate ('mediaGrading' or 'coverGrading').
        sim_params (dict): A dictionary with simulation parameters for each grade.
        formula_type (str): The type of formula ('cosine', 'parabola', 'linear', 'logarithmic').
    """
    if not result:
        print(f"--- Skipping simulation plot for {grading_col} because model failed. ---")
        return
    ...
    # 2. Map simulation parameters to the grid
    grid['D_base'] = grid[grading_col].map(sim_params['D_base'])
    grid['Scaling_Factor'] = grid[grading_col].map(sim_params['Scaling_Factor'])
    # Default to 1 (no reversal) if not provided
    grid['reverse_sign'] = grid[grading_col].map(sim_params.get('reverse_sign', {g: 1 for g in grades}))
    
    quintile_bins = pd.qcut(df['BP'], q=5, retbins=True)[1]
    grid['PriceGroupNum'] = np.digitize(grid['BP'], bins=quintile_bins)

    # --- Apply the selected formula ---
    if formula_type == 'cosine':
        price_adjustment = grid['Scaling_Factor'] * np.cos(((grid['PriceGroupNum'] - 1) / 4) * np.pi)
    elif formula_type == 'parabola':
        grid['Horizontal_Shift'] = grid[grading_col].map(sim_params['Horizontal_Shift'])
        price_adjustment = grid['Scaling_Factor'] * (grid['PriceGroupNum'] - grid['Horizontal_Shift'])**2
    elif formula_type == 'linear':
        # Center the linear effect around Q3 by default
        grid['Horizontal_Shift'] = grid[grading_col].map(sim_params.get('Horizontal_Shift', {g: 3 for g in grades}))
        price_adjustment = grid['Scaling_Factor'] * (grid['PriceGroupNum'] - grid['Horizontal_Shift'])
    elif formula_type == 'logarithmic':
        # Center the log effect around Q3 by default
        grid['Horizontal_Shift'] = grid[grading_col].map(sim_params.get('Horizontal_Shift', {g: 3 for g in grades}))
        # Use clip to prevent log(0) or log(negative) errors
        log_argument = (grid['PriceGroupNum'] / grid['Horizontal_Shift']).clip(lower=0.001)
        price_adjustment = grid['Scaling_Factor'] * np.log(log_argument)
    else:
        raise ValueError("formula_type must be 'cosine', 'parabola', 'linear', or 'logarithmic'")

    # Apply the reversal sign and combine with base discount
    final_adjustment = (grid['D_base'] + (price_adjustment * grid['reverse_sign']))
    grid['BP_Adjusted'] = grid['BP'] - final_adjustment
    
    # 3. Calculate NEW predicted probabilities
    predict_df_adjusted = grid[['BP_Adjusted', grading_col]].rename(columns={'BP_Adjusted': 'BP'})
    grid['Prob_Adjusted'] = result.predict(predict_df_adjusted)

    # 4. Plot both scenarios
    plt.figure(figsize=(14, 8))
    plot_grading_order = ['NM', 'EX', 'VG', 'G']
    
    sns.lineplot(data=grid, x='BP', y='Prob_Original', hue=grading_col, hue_order=plot_grading_order, legend='full')
    sns.lineplot(data=grid, x='BP', y='Prob_Adjusted', hue=grading_col, hue_order=plot_grading_order,
                 linestyle='--', legend=False)

    title_str = grading_col.replace('Grading', ' Grading')
    plt.title(f'Simulated Impact of {formula_type.capitalize()} Price Adjustment on {title_str}', fontsize=18, pad=20)
    plt.xlabel('Original Base Price ($)', fontsize=12)
    plt.ylabel('Predicted Probability of Sale', fontsize=12)
    plt.grid(True, linestyle='--', alpha=0.6)
    
    from matplotlib.lines import Line2D
    handles, labels = plt.gca().get_legend_handles_labels()
    custom_lines = [Line2D([0], [0], color='black', lw=2), Line2D([0], [0], color='black', lw=2, linestyle='--')]
    plt.legend(handles=handles[:len(plot_grading_order)] + custom_lines,
               labels=labels[:len(plot_grading_order)] + ['Original Prob.', 'Adjusted Prob.'])
